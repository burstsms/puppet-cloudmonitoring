class cloudmonitoring::token {
  ...
  exec { "raxmon-agent-tokens-create":
    command     => "raxmon-agent-tokens-create --label=${::hostname}",
    path        => ["/usr/local/bin", "/usr/bin/", "/bin"],
    environment => "RAXMON_RAXRC=${cloudmonitoring::raxmon::raxrc}",
    require     => Package["rackspace-monitoring-cli"],
    unless      => "raxmon-agent-tokens-list | grep -i ${::hostname}",
  }

  # now that the token exists, write the token to a file
  $agent_token_filename = "/tmp/${::hostname}.agent_token"
  exec { "raxmon-agent-tokens-list ${::hostname}":
    command     => "raxmon-agent-tokens-list | grep -i ${::hostname} | sed -n 's/.*id=\\([0-9a-f]*\\.[0-9]*\\).*/\\1/p' > ${agent_token_filename}",
    path        => ["/usr/local/bin", "/usr/bin/", "/bin"],
    environment => "RAXMON_RAXRC=${cloudmonitoring::raxmon::raxrc}",
  ...
  exec { "create ${cfg_filename}":
    command => "echo -n 'monitoring_token ' > ${cfg_filename} && cat ${agent_token_filename} >> ${cfg_filename} && echo 'monitoring_id ${::hostname}' >> ${cfg_filename}",
    path    => ["/usr/bin/", "/bin"],
    creates => $cfg_filename,
    require => Exec["raxmon-agent-tokens-list ${::hostname}"],
    notify  => Class['cloudmonitoring::service'],
  }

  # get the agent id for this provider
  $agent_id_filename = "/tmp/${::hostname}.agent_id"
  exec { "create ${agent_id_filename}":
    command     => "raxmon-entities-list | grep -i ${::hostname} | sed -n 's/.*id=\\([a-zA-Z0-9]*\\) .*/\\1/p' > ${agent_id_filename}",
    path        => ["/usr/local/bin", "/usr/bin/", "/bin"],
    environment => "RAXMON_RAXRC=${cloudmonitoring::raxmon::raxrc}",
  ...
  exec { "raxmon-entities-update --id=$(cat ${agent_id_filename}) --agent-id=${::hostname}":
    path        => ["/usr/local/bin", "/usr/bin/", "/bin"],
    environment => "RAXMON_RAXRC=${cloudmonitoring::raxmon::raxrc}",
